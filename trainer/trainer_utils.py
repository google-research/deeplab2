...
class AdamWeightDecay(tf.keras.optimizers.legacy.Adam):
  """Adam enabled L2 weight decay and clip_by_global_norm on gradients.
  ...
  def _decay_weights_op(self, var, learning_rate, apply_state):
    do_decay = self._do_weight_decay(var.name)
    logging.info('weight decay for %s is %s.', var.name, do_decay)
    if do_decay:
  ...
  def apply_gradients(self,
    ...
    if experimental_aggregate_gradients and self.gradient_clip_norm > 0.0:
      ...
      # longer implicitly allreduce gradients, users manually allreduces
  ...
  def _do_weight_decay(self, param_name):
