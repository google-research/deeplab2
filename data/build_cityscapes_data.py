...
download the dataset, and (2) run the script provided by Cityscapes to generate
the training groundtruth (sample commandline below).

```bash
  python cityscapesscripts/preparation/createTrainIdLabelImgs.py

  # For generate panoptic groundtruth, run the following commands.
...
The expected directory structure should be as follows:
...
Also note that this script and tensorflow model will use `TrainId` instead of
of `EvalId` used on the evaluation server. Thus, the users need to convert
the predicted labels to `EvalId` for evaluation on the server.
...
For semantic segmentation, the encoded segmentation map will be the same as PNG
file created by `createTrainIdLabelImgs.py`.

For panoptic segmentation, the encoded segmentation map will be the raw bytes
of a int32 panoptic map, where each pixel is assigned to a panoptic ID.
Unlike the ID used in Cityscapes script (`json2instanceImg.py`), this panoptic
ID is computed by:
